...
mysql.exe -u root --port=3307 -e "ALTER TABLE absence CONVERT TO CHARACTER SET utf8;" gpec_mysql_embedded
mysql.exe -u root --port=3307 -e "ALTER TABLE contrat_travail CONVERT TO CHARACTER SET utf8;" gpec_mysql_embedded
mysql.exe -u root --port=3307 -e "ALTER TABLE fichemetierentreprise CONVERT TO CHARACTER SET utf8;" gpec_mysql_embedded
mysql.exe -u root --port=3307 -e "ALTER TABLE gpec_user CONVERT TO CHARACTER SET utf8;" gpec_mysql_embedded
mysql.exe -u root --port=3307 -e "ALTER TABLE objectifsentretien CONVERT TO CHARACTER SET utf8;" gpec_mysql_embedded

echo "for"
set line=""
for %%i in %table_list% do (
	mysql.exe -u root --port=3307 -e "SELECT !%%i_column_list! into OUTFILE './../bin/%%i.csv' FIELDS TERMINATED BY '|' LINES TERMINATED BY '/eol/\n' FROM %%i x;" gpec_mysql_embedded
	FOR /F "tokens=* delims=/eol/" %%x in (%%i.csv) DO (
		echo %%x
		set str=%%x
		if "!str:~-5!" equ "/eol/" (
			echo "contient eol"
			...
			echo %%i^|!str:\N=null! >> root_export.csv
		) else (
			echo "ne contient pas eol"
			echo *!line!
			if not !line!=="" (
				set "line=!line!%%x"
			) else (
