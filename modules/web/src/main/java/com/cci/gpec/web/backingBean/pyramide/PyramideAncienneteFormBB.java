...
public class PyramideAncienneteFormBB {
	...
	public PyramideAncienneteFormBB() throws Exception {
		init();
	}
	...
	public void init() throws Exception {
		...
		imagen = new ByteArrayResource(pyramideService.creerPyramide(
				"Ancienneté", null, null, null, 0,
	...
	public void recalculePyramide(ValueChangeEvent event) throws Exception {
		...
		if (event.getComponent().getId().equals("entrepriseListPyramideA")) {
			...
			if (event.getComponent().getId().equals("idServiceListPyramideA")) {
				...
				if (event.getComponent().getId().equals("metierListPyramideA")) {
	...
	private void createImagePyramideAnciennete() throws Exception {
		...
		PyramideServiceImpl pyramideService = new PyramideServiceImpl();
		imagen = new ByteArrayResource(pyramideService.creerPyramide(
				"Anciennet\u00E9", idEntreprise, idService, idMetier, 0,
				Integer.parseInt(session.getAttribute("groupe").toString())));
	...
	public ArrayList<SelectItem> getMetierList() throws Exception {
		if (this.idEntrepriseSelected > 0) {
			if (this.idServiceSelected > 0) {
				List<MetierBean> metierBeanList = getMetiersList();
				SelectItem selectItem;
				metierList = new ArrayList<SelectItem>();
